...
async function searchOnline(item, budget, currency, location) {
  ...
  url.searchParams.set('gl', gl); // country -> correct currency
  url.searchParams.set('location', location); // city-level localization
...
async function runSearch({ item, budget, currency = 'USD', location }) {
  ...
  const [onlineResult, nearbyResult] = await Promise.allSettled([
    searchOnline(item, budget ? Number(budget) : null, currency, location),
