...
// preference, marketplace filtering) â leave it undefined for categories
...
// clothing/shoes â a "men's" desk lamp would be nonsense, so gender is
...
// aren't how people actually buy clothes â filtered out for apparel only.
...
// ---------- nearby (SerpAPI Google Local â no Google Cloud billing needed) ----------
...
async function searchNearby(item, location, gender, categoryInfo) {
  ...
  // item â that's what actually finds real shops on Google Local.
...
async function searchOnline(item, budget, currency, location, gender, categoryInfo) {
  ...
  if (apparel) {
    ...
    // clutter â this is what makes brand-appropriate results possible.
    if (parsed.length < 6) {
      const tiers = RETAILER_TIERS[categoryInfo.category];
  ...
  // pricier ones worth stretching for â not just everything crammed under
